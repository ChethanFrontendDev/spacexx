...
function Navbar({ navData }) {
  ...
  return (
    <div
      className={`z-10 w-full h-fit fixed top-0 left-0 text-white bg-transparent flex justify-center items-end p-5 lg:p-2 lg:pt-6 ${
        isVisible ? "opacity-100" : "opacity-0"
      } transition-opacity duration-300`}
    >
      <div className="left-container w-[90%] flex justify-center lg:w-[240px] lg:h-[38px] lg:ml-10">
        <Link to={"/"}>
          <svg
            className="w-40 h-auto lg:w-full"
            version="1.1"
            x="0px"
            y="0px"
            viewBox="0 0 400 50"
      ...
      <div className="center-container hidden lg:w-[60%] lg:flex gap-4 ml-10 whitespace-nowrap">
        {navData.map((item, index) => (
          <Link to={item.path} key={index}>
            <span
              className="uppercase font-semibold text-sm relative transition-all duration-300 ease-in-out 
            after:content-[''] after:absolute after:left-0 after:-bottom-1 after:w-0 after:h-[0.5px] after:bg-zinc-400 after:transition-all after:duration-500 hover:after:w-full"
            >
              {item.name}
            </span>
          </Link>
      ...
      <div className="right-container w-fit flex justify-end gap-7 lg:w-[12%]">
        <Link to={"/shop"}>
          <span
            className="hidden uppercase text-sm font-semibold lg:flex lg:items-end relative transition-all duration-300 ease-in-out 
      after:content-[''] after:absolute after:left-0 after:-bottom-1 after:w-0 after:h-[0.5px] after:bg-zinc-400 after:transition-all after:duration-500 hover:after:w-full"
          >
            Shop
          </span>
